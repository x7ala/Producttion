    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div>
                ...
                <form method="POST" action="{{ url('update-services/'.$services->id) }}">
                    {{ @csrf_field() }}

                    <div>
                        <label class="block font-medium text-sm text-gray-700" for="email">Başlık</label>
                        <input type="text" class="form-control" name="title" required value="{{$services->title}}">
                    </div>

                    <div>
                        <label class="block font-medium text-sm text-gray-700" for="email">Özet</label>
                        <textarea name="summary" class="form-control" required >{{$services->summary}}</textarea>
                    </div>

                    <div>
                        <label class="block font-medium text-sm text-gray-700" for="email">Açıklama</label>
                        <textarea name="content" id="content" class="form-control" required >{{$services->content}}</textarea>
                    </div>

                    <div>
                        <label class="block font-medium text-sm text-gray-700" for="email">Sıra</label>
                        <input type="text" class="form-control" name="sort_order" required value="{{$services->sort_order}}">
                    </div>
